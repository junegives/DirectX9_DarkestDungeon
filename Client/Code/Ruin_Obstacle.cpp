...
void CRuin_Obstacle::GetInteractionKey(const _float& fTimeDelta)
{
	...
	if (GetAsyncKeyState('C') & 0x8000)
	{
		...
		if (dynamic_pointer_cast<CPlayer>(CGameMgr::GetInstance()->GetPlayer())->GetCurrentItem()
			== EHandItem::SHOVEL)
		{
			CSoundMgr::GetInstance()->PlaySound(L"prop_obstacle_shipwreck_pick.wav", CHANNELID::OBSTACLE, 1.f);
			CSoundMgr::GetInstance()->PlaySound(L"prop_curio_use_shovel.wav", CHANNELID::INTERACTION, 1.f);
		}
		// 삽없는 상호작용
		else
		{
			CSoundMgr::GetInstance()->PlaySound(L"prop_obstacle_shipwreck_hand.wav", CHANNELID::OBSTACLE, 1.f);
		}
